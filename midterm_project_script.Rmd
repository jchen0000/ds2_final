...
pred_test <- nn %>% predict(x2)
nn.pred = pred_test[,2]

# Confusion Matrix
confusionMatrix(data = glmn.pred, reference = y2, positive = "vacc")
confusionMatrix(data = gam.pred, reference = y2, positive = "vacc")
confusionMatrix(data = lda.pred, reference = y2, positive = "vacc")
confusionMatrix(data = rf.pred, reference = y2, positive = "vacc")
confusionMatrix(data = svml.pred, reference = y2, positive = "vacc")
confusionMatrix(data = svmr.pred, reference = y2, positive = "vacc")
...
auc <- c(roc.glmn$auc[1], roc.gam$auc[1], roc.lda$auc[1],roc.rf$auc[1],roc.svml$auc[1],roc.svmr$auc[1], roc.nn$auc[1])

modelNames <- c("glmn","gam","lda","rf","svml","svmr","nn")

# fig 5
ggroc(list(roc.glmn, roc.gam, roc.lda, roc.rf, roc.svml, roc.svmr, roc.nn), legacy.axes = TRUE) + 
